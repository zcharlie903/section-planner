  ...
  async function handleSubmit(e) {
    ...
    try {
      const payload = {
        ...
        returnRate,
        inflationRate,
        // legacy flat fields your backend v2 can accept
        contribAfterTax,
        contribTaxDeferred,
        contribTaxFree,
        pensionStart,
      ...
      // --- Normalize backend fields → keep old UI happy ---
      // Prefer backend-provided values if present; compute fallbacks when missing.
      const balanceAtRet =
        (Array.isArray(data.path) &&
          data.path.find((row) => row.age === retirementAge)?.balance) ?? null;

      const finalSavings =
        typeof data.finalSavings === "number"
          ? data.finalSavings
          : typeof data.finalSavingsAtMaxAge === "number"
          ? data.finalSavingsAtMaxAge
          : 0;

      const safeWithdraw =
        typeof data.safeWithdraw === "number"
          ? data.safeWithdraw
          : Math.round(((balanceAtRet ?? finalSavings) || 0) * 0.04);

      setResult({
        retireAge: data.retireAge ?? null,
        finalSavings,
        safeWithdraw,
      });
    } catch (err) {
